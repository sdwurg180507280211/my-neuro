...
const { VolcEngineStreamingASR } = require('../../voice/volcengine-streaming-asr.js');
...
class ASRController {
    constructor(vadUrl, asrUrl, config, inputRouter, diaryManager) {
        ...
        // 检查是否使用火山引擎ASR或百度流式ASR
        this.useVolcEngineASR = config.cloud?.volc_asr?.enabled === true;
        this.useBaiduStreamingASR = config.cloud?.baidu_asr?.enabled === true;

        // 检查ASR是否可用（本地ASR/火山ASR/百度ASR任一启用即可）
        const localASREnabled = config.asr?.enabled !== false;
        this.asrEnabled = localASREnabled || this.useVolcEngineASR || this.useBaiduStreamingASR;
        this.voiceBargeInEnabled = config.asr?.voice_barge_in || false;

        console.log(`语音打断功能: ${this.voiceBargeInEnabled ? '已可用' : '已禁用'}`);
        console.log(`火山引擎ASR: ${this.useVolcEngineASR ? '已启用' : '已禁用'}`);
        ...
        // 根据配置选择ASR处理器
        if (this.useVolcEngineASR) {
            // 使用火山引擎ASR
            console.log('✅ 使用火山引擎流式ASR');
            this.asrProcessor = new VolcEngineStreamingASR(config);
            // 设置实时字幕回调
            this.setupInterimResultCallback();
        } else if (this.useBaiduStreamingASR) {
            // 使用百度流式ASR
    ...
    setupASRCallback() {
        ...
        const flush = async (showSubtitle) => {
            ...
            // 🔥 例外：百度流式 ASR 和火山引擎 ASR 不应该被 asrLocked 阻塞，因为它们的识别结果已经确定了
            const isBaiduASR = this.asrProcessor && this.asrProcessor.constructor.name === 'BaiduStreamingASR';
            const isVolcASR = this.asrProcessor && this.asrProcessor.constructor.name === 'VolcEngineStreamingASR';
            if ((this.asrProcessor?.isRecording || this.asrProcessor?.asrLocked) && !isBaiduASR && !isVolcASR) {
        ...
        this.asrProcessor.setOnSpeechRecognized(async (text) => {
            ...
            if (isBaiduASR || isVolcASR) {
                // 火山引擎/百度流式 ASR：收到 definite 结果立即处理，不需要等合并窗口
                console.log('⚡ 火山引擎/百度ASR，立即提交');
                fs.appendFileSync('/Users/edy/asr-debug.log', `[${new Date().toISOString()}] 立即提交\n`);
                await flush(showSubtitle);
            } else {
                // 本地 ASR：使用合并窗口
